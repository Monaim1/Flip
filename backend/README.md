...
- `POST /api/query/epic-stream` runs voice + UI agents in parallel (EPIC) and streams UI command events without blocking voice responses.
...
- `FINANCE_DB_PATH` (default: first existing path among `data/finance.db`, `backend/finance.db`, `backend/data/finance.db`)
...
### `POST /api/query/epic-stream`
SSE stream with mixed event types:
- Voice: `content`, `step`, `result`, `error`
- UI: `ui_command`, `ui_intent`, `ui_state`, `ui_error`

UI events are generated by a dedicated lightweight UI agent that tracks deferred intents
and can apply UI updates later when voice-agent results provide missing parameters.
